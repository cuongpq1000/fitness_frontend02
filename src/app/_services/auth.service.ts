...
export class AuthService {
  ...
  login(username: string, password: string): Observable<any> {
    ...
    return this.http.post<any>(`http://localhost:8102/user/authenticate`, { username, password })
      .pipe(map(user => {
        // login successful if there's a jwt token in the response
        if (user && user.token) {

          localStorage.setItem('currentUser', JSON.stringify(user.accountEntity));
          localStorage.setItem('token', user.token);
          localStorage.setItem('id', user.accountEntity.id);
